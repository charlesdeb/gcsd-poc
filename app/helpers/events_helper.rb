...
module EventsHelper
  def session_type_tab(session_type, position)
    ...
    link_to link_content, '#',
            class: "#{color_class} first:rounded-l-lg last:rounded-r-lg group relative min-w-0 flex-1 overflow-hidden bg-orange-50 py-4 px-4 text-sm font-medium text-center hover:bg-orange-200 focus:z-10"
  end
end
